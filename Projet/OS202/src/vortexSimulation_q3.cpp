...
int main(int argc, char *argv[])
{
    ...
    // taille nuage de points partiel pour chaque processus
    int partialCloudSize = cloud.numberOfPoints() / (size-1);
    int offsets[size], recvcounts[size];
    offsets[0] = 0;
    recvcounts[0] = 0;
    for (int i = 1; i < size; i++)
    {
        recvcounts[i] = 2* partialCloudSize;
        offsets[i] = 2 * partialCloudSize * (i-1);
    }
    ...
    if (rank == 0)
    {
        ...
        while (myScreen.isOpen())
        {
            ...
            if (animate || nextFrame){
                ...
                // reçoit et join les parties calculés pour chaque processus
                MPI_Gatherv(cloud.data(), 2* partialCloudSize, MPI_DOUBLE, cloud.data(), recvcounts, offsets, MPI_DOUBLE, 0 , MPI_COMM_WORLD);
                if(isMobile){
                    MPI_Recv(grid.data(), 2 * grid.numberOfPoints(), MPI_DOUBLE, 1, FLAG_GRID, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
                    MPI_Recv(vortices.data(), vortices.numberOfVortices() * 3, MPI_DOUBLE, 1, FLAG_VORTICE, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
                }
                nextFrame = false;
                frames++;
            }
    ...
    if (rank != 0)
    {
        // nuage de points particulière a chaque processus, sous-ensemble
        Geometry::CloudOfPoints processusCloud(partialCloudSize);

        for (int iCord = 0; iCord < partialCloudSize; iCord++) {
            processusCloud[iCord] = cloud[partialCloudSize * (rank - 1) + iCord];
        }        
        ...
        while (true)
        {
            ...
            // reçoit commande
            MPI_Recv(&key, 1, MPI_CHAR, 0, 0, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
            switch (key)
            {
            case 'E':
                MPI_Finalize();
            ...
            if(calculate){
                if (animate | advance)
                {
                    processusCloud = Numeric::solve_RK4_fixed_vortices(dt, grid, processusCloud);            
                    if(isMobile){
                        Numeric::updateVortices(dt, grid, vortices, processusCloud);
                    }
                    MPI_Gatherv(processusCloud.data(), 2* partialCloudSize, MPI_DOUBLE, cloud.data(), recvcounts, offsets,MPI_DOUBLE,0 , MPI_COMM_WORLD);
